...
return{init:function(g){var f=this;
this._badgeNumber=++STMBLPN.Widget.NUMBER;
this.selfContainer=false;
this.domain=g.domain?g.domain.replace(/^\.+/g,""):"stumbleupon.com";
this.type=g.type||"badge";
this.layout=g.layout?parseInt(g.layout):1;
this.layout=this.layout||1;
this.id=g.id||"stmblpn-widget-"+this._badgeNumber;
this.container=g.container;
if(g.id){var h=STMBLPN.byId(g.id);
if(h){this.selfContainer=true
}}switch(this.type){case"badge":this.location=g.location||document.URL;
break;
case"follow":this.ref=g.ref;
break
}return this
